...
public class ItemBlockFlatColored extends BlockItem {
    ...
    public ItemBlockFlatColored(final Block block) {
    ...
    private String getColorPrefix(final Set<EnumFlatColorAttributes> which) {
    ...
    private String getColorHueName(final Set<EnumFlatColorAttributes> characteristics) {
    ...
    public ITextComponent getDisplayName(@Nonnull ItemStack stack) {
        ...
        return new StringTextComponent(type + ModUtil.translateToLocal(prefix + hue + ".name") + " "
                + ModUtil.translateToLocal("flatcoloredblocks.Shade.name") + shadeNum);
    }
    ...
    public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        ...
        if (FlatColoredBlocks.instance.config.showLight && blk.lightValue > 0) {
            String sb = ModUtil.translateToLocal("flatcoloredblocks.tooltips.lightvalue")
                    + ' '
                    + blk.lightValue
                    + ModUtil.translateToLocal("flatcoloredblocks.tooltips.lightValueUnit");
        ...
        if (FlatColoredBlocks.instance.config.showOpacity && blk.opacity < 100) {
            String sb = ModUtil.translateToLocal("flatcoloredblocks.tooltips.opacity")
                    + ' '
                    + blk.opacity
                    + ModUtil.translateToLocal("flatcoloredblocks.tooltips.percent");
            tooltip.add(new StringTextComponent(sb));
        }
    ...
    private void addColor(final ColorFormat Format, final int value, final List<ITextComponent> tooltip) {
        ...
        if (Format == ColorFormat.HEX) {
            sb.append(ModUtil.translateToLocal("flatcoloredblocks.tooltips.hex"))
                    .append(' ');
            sb.append("#")
                    .append(hexPad(Integer.toString(r_h, 16)))
                    .append(hexPad(Integer.toString(g_s, 16)))
                    .append(hexPad(Integer.toString(b_v, 16)));
        } else if (Format == ColorFormat.RGB) {
            sb.append(ModUtil.translateToLocal("flatcoloredblocks.tooltips.rgb"))
                    .append(' ');
            ...
            sb.append(ModUtil.translateToLocal("flatcoloredblocks.tooltips.hsv"))
                    .append(' ');
    ...
    public int getColorFromItemStack(@Nonnull final ItemStack stack, final int renderPass) {
        final BlockState state = ModUtil.getFlatColoredBlockState(getColoredBlock(), stack);

...
public class ItemColoredBlockCrafter extends Item {
    ...
    public ActionResult<ItemStack> onItemRightClick(final World worldIn, final PlayerEntity playerIn, final Hand hand) {
        ...
        if (playerIn instanceof ServerPlayerEntity) {
            NetworkHooks.openGui(
                    ...
                    ModGuiTypes.colored_crafter.create(playerIn, worldIn, 0, 0, 0),
                    (BlockPos) null);
        }
    ...
    public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        if (scrollIndex == -1 && worldIn != null) {
            ...
            options.clear();
            options.addAll(
                    ItemTags.createOptional(new ResourceLocation(FlatColoredBlocks.instance.config.solidCraftingBlock))
                            .getAllElements());
            options.addAll(ItemTags.createOptional(
                            new ResourceLocation(FlatColoredBlocks.instance.config.glowingCraftingBlock))
                    .getAllElements());
            options.addAll(ItemTags.createOptional(
                            new ResourceLocation(FlatColoredBlocks.instance.config.transparentCraftingBlock))
                    .getAllElements());
        }
        ...
        if (!options.isEmpty() && scrollIndex >= 0) {
            ...
            tooltip.add(new TranslationTextComponent(
                    "item.flatcoloredblocks.coloredcraftingitem.tip1", it.getDisplayName(it.getDefaultInstance())));
        }
